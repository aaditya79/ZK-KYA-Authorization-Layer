...
router.post("/issue-credential", async (req, res) => {
  try {
    ...
    const expiry = req.body.expiry || Number(latestBlock.timestamp) + 3600;
...
router.post("/authorize-payment", async (req, res) => {
  try {
    ...
    const inputPath = path.join(BUILD_DIR, "api_auth_input.json");
    const witnessPath = path.join(BUILD_DIR, "api_auth_witness.wtns");
    const proofPath = path.join(BUILD_DIR, "api_auth_proof.json");
    const publicPath = path.join(BUILD_DIR, "api_auth_public.json");
